...
$queryBukuDipinjam = "
    SELECT 
        ...
        p.statuspengembalian
    FROM labib_buku b
    LEFT JOIN labib_peminjaman p ON b.bukuid = p.bukuid 
        AND p.userid = $userid
        AND p.tanggalpeminjaman = (
            SELECT MAX(p2.tanggalpeminjaman) 
            FROM labib_peminjaman p2 
            WHERE p2.bukuid = p.bukuid 
    ...
    WHERE p.statuspengembalian = 'belum dikembalikan'
    GROUP BY b.bukuid, b.judul, b.penulis, b.penerbit, b.tahunterbit, b.cover, p.tanggalpeminjaman, p.tanggalpengembalian, p.statuspengembalian;
";

$resultBukuDipinjam = mysqli_query($conn, $queryBukuDipinjam);

if (!$resultBukuDipinjam) {
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="product__page__content">
                    ...
                    <div class="row">
    <?php
    if ($resultBukuDipinjam->num_rows > 0) {
        while ($row = $resultBukuDipinjam->fetch_assoc()) {
            $judul = $row['judul'];
            $cover = $row['cover'];
            $bukuid = $row['bukuid'];
